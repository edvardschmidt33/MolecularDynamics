...
def langevin_verlet(init_pos, init_vel, box, m, ts, temp, cutoff, damp, epsilon, sigma, n_therm):
    ...
    for _ in tqdm(range(n_therm)):
        ...
        xi1 = np.random.normal(0.0, 1.0, size= (n_atoms, 3))
        xi2 = np.random.normal(0.0, 1.0, size= (n_atoms, 3))
        ...
        vel2 = vel1 + a2*ts_2
        vel = c1 * vel2 + c2 * xi2
        vel -= vel.mean(axis=0) #minimize drift
        f = f2
...
if __name__ == '__main__':
    ...
    vel = init_velocity(pos, m, Temp_target, seed=0)
